// Appends one fresh outrage story to src/data/outrage.ts on each run.
// Designed to be run by an hourly GitHub Actions cron and committed back.
// No third-party APIs required; uses a pool of kickers + templates to keep
// content distinct, relatable, and policy-safe.
...
const OUTRAGE_PATH = resolve(ROOT, "src/data/outrage.ts");
...
const KICKERS = [
  ...
  },
  {
    kicker: "Work",
    emoji: "💼",
    gradient: "from-neon-red/30 via-ink-900 to-ink-950",
    tag: "work",
  },
  {
    kicker: "Tech",
  ...
  },
  {
    kicker: "Money",
    emoji: "💸",
    gradient: "from-neon-red/25 via-ink-900 to-ink-950",
    tag: "money",
  },
  {
    kicker: "Food",
...
const TEMPLATES = [
  {
    ...
    videoQueries: [
      "hidden fees",
      "junk fees customer complaint",
      "fees customers angry",
    ],
  ...
  },
  {
    ...
    videoQueries: [
      "app update redesign rage",
      "bad ui redesign",
      "app users angry update",
    ],
  ...
  },
  {
    ...
    videoQueries: [
      "customer service hold forever",
      "long customer service wait",
      "hold music customer angry",
    ],
    tags: ["customer-service", "daily-rage", "corporate"],
  },
  {
    title: (t) => `Delivery App Raises Minimum Tip to Confusing New Amount`,
    description: (t) =>
    ...
    videoQueries: [
      "delivery app tipping too high",
      "tip creep prompt",
      "food delivery fees ridiculous",
    ],
  ...
  },
  {
    ...
    videoQueries: [
      ...
      "streaming service increases price",
      "subscription raise prices customer angry",
    ],
...
function buildStory() {
  ...
  const description = tmpl.description(ctx);
  const intro = tmpl.intro(ctx);
  const takes = tmpl.takes(ctx);
  const blame = tmpl.blame(ctx);
  const conclusion = tmpl.conclusion(ctx);
  const videoQuery = pick(tmpl.videoQueries);

  const nowISO = new Date().toISOString();
  const dateStamp = nowISO.slice(0, 10);
  const base = slugify(title);
  const slug = `${base}-${dateStamp}-${Math.floor(
...
function serialize(story) {
  ...
  lines.push(
    `    source: { name: ${tsString(story.source.name)}, url: ${tsString(
      story.source.url
    )} },`
  );
  lines.push(`    videoQuery: ${tsString(story.videoQuery)},`);
  lines.push(
    `    tags: [${story.tags.map((t) => tsString(t)).join(", ")}],`
  );
...
function main() {
  const src = readFileSync(OUTRAGE_PATH, "utf8");
  const marker = "export const OUTRAGE_STORIES: OutrageStory[] = [";
  const idx = src.indexOf(marker);
  if (idx === -1) {
    console.error("[outrage] marker not found in outrage.ts");
    process.exit(1);
  }
  const insertAt = idx + marker.length;
  const story = buildStory();
  const serialized = `\n${serialize(story)}`;
  const updated = src.slice(0, insertAt) + serialized + src.slice(insertAt);
  writeFileSync(OUTRAGE_PATH, updated, "utf8");
  console.log(`[outrage] appended story: ${story.slug}`);
}
